 ...
 */
public class Player extends JEDI {
	...
	public Player(Team team, int hitpoints, MessageRenderer m, SWWorld world) {
		...
		//this.capabilities.add(Capability.REPAIR);
	...
	public void act() {	
		...
		SWActionInterface action = SWGridController.getUserDecision(this);
		scheduler.schedule(action, this, 1);
		/*if (this.gethasDroid() && (action instanceof Move)){
			this.getDroid().say( "I am " + this.getDroid().getShortDescription() + " and I am following " + this.getShortDescription());
			scheduler.schedule(action, this.getDroid(), 1);
		}*/
		
		if (action instanceof Move){
			ArrayList<SWActor> cloneFollowers = this.clonefollowers();
			for (SWActor a : cloneFollowers){
				scheduler.schedule(action, a, 1);
			}
